...
// http://localhost:3000/ - display the homepage of proximity lab
...
app.post("/", (request, response) => {
	...
	console.log(email);
	connection.query('INSERT INTO subscribers (email, date) VALUES (?,?)', [email, inputDate], (err, result)=>{
		if (err){
			console.log(err)
		}
		response.redirect('/');
...
app.get(['/home','/login/home'], (request, response) => isLoggedin(request, settings => {
...
app.get('/admiin/', (request, response) => isAdmin(request, settings => {
	// Retrieve statistical data
...
//subscribers
// http://localhost:3000/admin/account - Admin edit/create account
app.get(['/admin/subscriber', '/admin/subscriber/:id'], (request, response) => isAdmin(request, settings => {
	// Default page (Create/Edit)
    let page = request.params.id ? 'Edit' : 'Create';
	// Current date
	let d = new Date();
    ...
    // GET request ID exists, edit account
    if (request.params.id) {
		connection.query('SELECT * FROM subscribers WHERE id = ?', [request.params.id], (error, accounts) => {
			account = accounts[0];
			response.render('admin/subscriber.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: account });
		});
	} else {
		response.render('admin/subscriber.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: account });
	}
...
// http://localhost:3000/admin/account - Admin edit/create account
app.post(['/admin/subscriber', '/admin/subscriber/:id'], (request, response) => isAdmin(request, settings => {
    // GET request ID exists, edit account
    if (request.params.id) {
        // Edit an existing account
        page = 'Edit'
		...
		connection.query('SELECT * FROM subscribers WHERE id = ?', [request.params.id], (error, accounts) => {
			...
			if (request.body.submit) {
				// Update account details
				connection.query('UPDATE subscribers SET email = ?,  date = ? WHERE id = ?', [ request.body.email, request.body.registered,  request.params.id]);
				// Redirect to admin accounts page
				response.redirect('/admin/');
			} else if (request.body.delete) {
				// delete account
				response.redirect('/admin/subscriber/delete/' + request.params.id);
			}
		});
	} else if (request.body.submit) {
		// Hash password
		...
		// Create account
		connection.query('INSERT INTO subscribers (email,date) VALUES (?,?)', [request.body.email, request.body.registered]);
		// Redirect to admin accounts page
		response.redirect('/admin/accounts/msg1/n0/n0/n0/n0/ASC/id/1');
	}
}, () => {
...
// http://localhost:3000/admin/account/delete/:id - Delete account based on the ID param
app.get('/admin/subscriber/delete/:id', (request, response) => isAdmin(request, settings => {
    // GET request ID exists, delete account
    if (request.params.id) {
		connection.query('DELETE FROM subscribers WHERE id = ?', [request.params.id]);
		response.redirect('/admin/');
	}
}, () => {
	// Redirect to login page
...
// http://localhost:3000/admin/account - Admin edit/create account
app.get(['/admin/news', '/admin/news/:id'], (request, response) => isAdmin(request, settings => {
	// Default page (Create/Edit)
    let page = request.params.id ? 'Edit' : 'Create';
	// Current date
	let d = new Date();
    ...
    // GET request ID exists, edit account
    if (request.params.id) {
		connection.query('SELECT * FROM news WHERE id = ?', [request.params.id], (error, accounts) => {
			news = accounts[0];
			response.render('admin/news.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: news });
		});
	} else {
		response.render('admin/news.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: news });
	}
...
// http://localhost:3000/admin/account - Admin edit/create account
app.post(['/admin/news', '/admin/news/:id'], (request, response) => isAdmin(request, settings => {
    // GET request ID exists, edit account
    if (request.params.id) {
        // Edit an existing account
        page = 'Edit'
		...
		connection.query('SELECT * FROM news WHERE id = ?', [request.params.id], (error, accounts) => {
			...
			if (request.body.submit) {
				// Update account details
				connection.query('UPDATE news SET title = ?,  content = ?, link=?, date=? WHERE id = ?', [ request.body.title, request.body.content,request.body.link,request.body.date,  request.params.id]);
				// Redirect to admin accounts page
				response.redirect('/admin/');
			} else if (request.body.delete) {
				// delete account
				response.redirect('/admin/news/delete/' + request.params.id);
			}
		});
	} else if (request.body.submit) {
		// Hash password
		...
		// Create account
		connection.query('INSERT INTO news (title, content, link ,date) VALUES (?,?,?,?)', [request.body.title, request.body.content, request.body.link,request.body.date]);
		// Redirect to admin accounts page
		response.redirect('/admin/');
	}
}, () => {
...
// http://localhost:3000/admin/account/delete/:id - Delete account based on the ID param
app.get('/admin/news/delete/:id', (request, response) => isAdmin(request, settings => {
    // GET request ID exists, delete account
    if (request.params.id) {
		connection.query('DELETE FROM news WHERE id = ?', [request.params.id]);
		response.redirect('/admin/');
	}
}, () => {
	// Redirect to login page
...
// http://localhost:3000/admin/account - Admin edit/create account
app.get(['/admin/reports', '/admin/reports/:id'], (request, response) => isAdmin(request, settings => {
	// Default page (Create/Edit)
    let page = request.params.id ? 'Edit' : 'Create';
	// Current date
	let d = new Date();
    ...
    // GET request ID exists, edit account
    if (request.params.id) {
		connection.query('SELECT * FROM reports WHERE id = ?', [request.params.id], (error, accounts) => {
			news = accounts[0];
			response.render('admin/reports.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: news });
		});
	} else {
		response.render('admin/reports.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: news });
	}
    ...
    if (request.params.id) {
		...
		connection.query('SELECT * FROM reports WHERE id = ?', [request.params.id], (error, accounts) => {
			...
			if (request.body.submit) {
				...
				// delete account
				response.redirect('/admin/reports/delete/' + request.params.id);
			}
		});
	} else if (request.body.submit) {
		// Hash password
...
// http://localhost:3000/admin/account/delete/:id - Delete account based on the ID param
app.get('/admin/reports/delete/:id', (request, response) => isAdmin(request, settings => {
    // GET request ID exists, delete account
    if (request.params.id) {
		connection.query('DELETE FROM reports WHERE id = ?', [request.params.id]);
		response.redirect('/admin/');
	}
}, () => {
	// Redirect to login page
...
//Map Link
// http://localhost:3000/admin/account - Admin edit/create account
app.get(['/admin/maplink/','/admin/maplink/:id'], (request, response) => isAdmin(request, settings => {
	// Default page (Create/Edit)
    let page = request.params.id ? 'Edit' : 'Create';
	// Current date
	let d = new Date();
    ...
    // GET request ID exists, edit account
    if (request.params.id) {
		connection.query('SELECT * FROM map WHERE id = ?', [request.params.id], (error, accounts) => {
			maplink = accounts[0];
			response.render('admin/maplink.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: maplink });
		});
	} else {
		response.render('admin/maplink.html', { selected: 'accounts', selectedChild: 'manage', page: page, account: maplink });
	}
...
// http://localhost:3000/admin/account - Admin edit/create account
app.post('/admin/maplink/:id', (request, response) => isAdmin(request, settings => {
    // GET request ID exists, edit account
    if (request.params.id) {
        // Edit an existing account
        page = 'Edit'
		...
		connection.query('SELECT * FROM map WHERE id = ?', [request.params.id], (error, accounts) => {
			...
			if (request.body.submit) {
				// Update account details
				connection.query('UPDATE map SET link=? WHERE id = ?', [ request.body.link, request.params.id]);
				// Redirect to admin accounts page
				response.redirect('/admin/');
			} 
		});
