...
app.get('/admin/', (request, response) => isAdmin(request, settings => {
	// Retrieve statistical data
	connection.query('SELECT * FROM subscribers WHERE cast(date as DATE) = cast(now() as DATE) ORDER BY date DESC; SELECT COUNT(*) AS total FROM subscribers LIMIT 1;', (error, results, fields) => {
		// Render dashboard template
		response.render('admin/dashboard.html', { selected: 'dashboard', accounts: results[0], accounts_total: results[1][0], timeElapsedString: timeElapsedString });
	});
}, () => {
	// Redirect to login page
	response.redirect('/');
